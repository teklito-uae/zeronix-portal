...
/** Raw row shape returned by POST /admin/customers/import/preview, before the
 *  client-side `action` default is applied. */
type ParsedRowFromApi = Omit<ParsedRow, 'action'>;

interface ImportPreviewResponse {
  rows: ParsedRowFromApi[];
  conflict_count: number;
  total: number;
}

interface ImportCommitResponse {
  results: { created: number; merged: number; skipped: number; errors: string[] };
}
...
export const CompanyImport = () => {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await api.post<ImportPreviewResponse>(`/admin/customers/import/preview`, fd);
      const parsed: ParsedRow[] = res.data.rows.map((r: ParsedRowFromApi) => ({
        ...r,
      ...
      toast.error((err as AxiosError<{ message?: string }>).response?.data?.message || 'Preview failed');
    } finally {
  ...
  const handleCommit = async () => {
    ...
    try {
      const res = await api.post<ImportCommitResponse>(`/admin/customers/import/commit`, {
        rows,
        label_ids: labelIds,
      ...
      toast.error((err as AxiosError<{ message?: string }>).response?.data?.message || 'Import failed');
    } finally {
      setIsCommitting(false);
    }
  };

  const updateRow = <K extends keyof ParsedRow>(idx: number, field: K, value: ParsedRow[K]) => {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-brand-subtle">Supports .vcf (vCard) and .json formats</p>
        </div>
        <div
          ...
          <pre className="text-[10px] text-brand-secondary font-mono overflow-x-auto">{`[
  {
    ...
    "email": "ahmed@alnoor.ae"
      ...
      <div className="space-y-5">
        <div>
          <h3 className="text-sm font-bold text-brand-primary mb-1">2. Assign Labels</h3>
          <p className="text-xs text-brand-subtle">Tag this import for easy filtering and team assignment</p>
        </div>
        <LabelSelector selectedIds={labelIds} onChange={setLabelIds} />

        <div className="space-y-2">
          <h3 className="text-sm font-bold text-brand-primary">3. Assign to Staff (Optional)</h3>
          <p className="text-xs text-brand-subtle">All imported contacts will be assigned to this staff member</p>
          <Select value={assignUserId} onValueChange={setAssignUserId}>
            <SelectTrigger className="h-11 bg-brand-bg border-brand-border text-brand-primary rounded-xl">
              <SelectValue placeholder="Leave unassigned (assign later)" />
            </SelectTrigger>
            <SelectContent className="bg-brand-white border-brand-border text-brand-primary">
  ...
  const renderStep3 = () => (
    <div className="flex flex-col items-center text-center py-8 gap-6">
      ...
      <div>
        ...
        <p className="text-sm text-brand-subtle mt-1">Your contacts have been processed successfully.</p>
      </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-brand-primary tracking-tight">Import Contacts</h2>
          <p className="text-xs text-brand-subtle mt-0.5">Upload VCF or JSON to bulk-import companies with labels</p>
        </div>
      </div>
