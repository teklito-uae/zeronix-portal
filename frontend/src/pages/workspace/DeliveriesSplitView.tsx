...
/**
 * Zoho-Books-style master-detail layout for Deliveries, mirroring
 * QuotesSplitView.tsx: a compact scrollable left column list, and a right
 * column detail pane (DeliveryDetailView). Below `md:` only the list is
 * shown (row click navigates to the full-page editor instead).
 */
export const DeliveriesSplitView = () => {
  ...
  useEffect(() => {
    if (!selectedId && deliveries.length > 0) {
      setSearchParams({ id: String(deliveries[0].id) }, { replace: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [deliveries, selectedId]);

  const sendEmailMutation = useMutation({
    mutationFn: async (id: number) => (await api.post(`/admin/deliveries/${id}/send-email`)).data,
    onSuccess: () => {
  ...
  const handleRowClick = (row: Delivery) => {
    ...
    if (isDesktop) {
      ...
      navigate(`${getBasePath()}/deliveries/${row.id}`);
    }
  };
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 md:grid-cols-[360px_1fr]">
        ...
        <div className="flex flex-col min-h-0 border-r border-brand-border/50 bg-brand-white">
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              <PageLoader label="Loading deliveries..." iconSize={28} className="h-full min-h-[200px] gap-3" />
            ) : deliveries.length === 0 ? (
              <div className="flex items-center justify-center h-full min-h-[200px] px-4">
                <p className="text-[12px] text-brand-subtle text-center">No deliveries found.</p>
              </div>
            ) : (
              <ul>
                {deliveries.map((inv) => {
