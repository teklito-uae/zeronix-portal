...
/**
 * Zoho-Books-style master-detail layout for Invoices, mirroring
 * QuotesSplitView.tsx: a compact scrollable left column list, and a right
 * column detail pane (InvoiceDetailView). Below `md:` only the list is
 * shown (row click navigates to the full-page editor instead).
 */
export const InvoicesSplitView = () => {
  ...
  const { data: resourceData, isLoading } = useResourceList<PaginatedResponse<Invoice>>('invoices', {
  ...
  const invoices: Invoice[] = resourceData?.data || [];
  ...
  useEffect(() => {
    if (!selectedId && invoices.length > 0) {
      setSearchParams({ id: String(invoices[0].id) }, { replace: true });
    }
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 md:grid-cols-[360px_1fr]">
        ...
        <div className="flex flex-col min-h-0 border-r border-brand-border/50 bg-brand-white">
          ...
          <div className="px-4 py-3 flex items-center gap-2 border-b border-brand-border/50 flex-shrink-0">
            ...
            <Button
              size="icon"
              onClick={() => navigate(`${getBasePath()}/invoices/create`)}
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              <PageLoader label="Loading invoices..." iconSize={28} className="h-full min-h-[200px] gap-3" />
            ) : invoices.length === 0 ? (
              <div className="flex items-center justify-center h-full min-h-[200px] px-4">
                <p className="text-[12px] text-brand-subtle text-center">No invoices found.</p>
              </div>
            ) : (
              <ul>
                {invoices.map((inv) => {
