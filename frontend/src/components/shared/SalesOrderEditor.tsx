...
import type { Customer, CustomerContact, Product } from '@/types';
import { ArrowLeft, Save, Plus, Trash2, Loader2, Calendar, User, Settings2, Edit3, Pencil, ShieldCheck, PackageCheck } from 'lucide-react';
...
/**
 * Sales Order Editor — sits between an accepted Quotation and a Delivery.
 * No stock or financial side effects here; inventory only moves when a
 * Delivery created from this order is marked delivered.
 */
export const SalesOrderEditor = ({ id, isNew }: SalesOrderEditorProps) => {
  ...
  const docLabel = isNew ? 'New Sales Order' : (docData.order_number || `#${id}`);

  useBreadcrumb([
    { label: 'Sales Orders', href: `${getBasePath()}/sales-orders` },
    { label: docLabel },
  ]);
  ...
  const { data: contactsList = [] } = useQuery({
    queryKey: ['customers', docData.customer_id, 'contacts'],
    queryFn: async () => (await api.get(`/admin/customers/${docData.customer_id}/contacts?active=1`)).data as CustomerContact[],
    enabled: !!docData.customer_id,
  });

  useEffect(() => {
    if (!docData.customer_contact_id && contactsList.length > 0) {
      const primary = contactsList.find((c) => c.is_primary) || contactsList[0];
      setDocData((prev: any) => ({ ...prev, customer_contact_id: primary.id }));
    }
  }, [contactsList]);
  ...
  const handleSaveDoc = async () => {
    ...
    if (items.length === 0) return toast.error('Sales order must contain at least one line item.');
    setLoading(true);
    try {
      const payload = { ...docData, items };
      if (isNew) {
        await api.post('/admin/sales-orders', payload);
        toast.success('Sales order created successfully.');
      } else {
        await api.put(`/admin/sales-orders/${id}`, payload);
        toast.success(`Records updated for ${docLabel}.`);
      }
      queryClient.invalidateQueries({ queryKey: ['sales-orders'] });
      navigate(`${getBasePath()}/sales-orders`);
    } catch (err: any) {
  ...
  const handleCreateDelivery = async () => {
    setLoading(true);
    try {
      const res = await api.post(`/admin/sales-orders/${id}/convert-to-delivery`);
      toast.success('Delivery created from sales order.');
      navigate(`${getBasePath()}/deliveries/${res.data.id}`);
    } catch (err: any) {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <Button
            variant="outline"
            size="icon"
            onClick={() => navigate(`${getBasePath()}/sales-orders`)}
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-2xl font-bold text-admin-text-primary tracking-tight">
                {isNew ? 'New Sales Order' : `ORDER ${docData.order_number || '#' + id}`}
              </h1>
            ...
            <p className="text-xs font-medium text-admin-text-muted mt-0.5 uppercase tracking-wide opacity-70">
              {isNew ? 'Confirmed Sales Intent' : 'Managing Sales Order Records'}
            </p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          {!isNew && docData.status === 'confirmed' && (
            <Button
            ...
            >
              <PackageCheck size={16} className="mr-2" /> Create Delivery
            </Button>
          )}
          <Button
        ...
        <div className="xl:col-span-8 space-y-6">
          <div className="bg-admin-surface border border-admin-border rounded-lg p-6 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Select value={String(docData.customer_id || '')} onValueChange={(v) => setDocData({ ...docData, customer_id: Number(v), customer_contact_id: undefined })}>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-wider text-admin-text-muted ml-1 flex items-center gap-1.5">
                  <User size={12} className="text-zeronix-blue" /> Attention (Contact)
                </Label>
                <Select
                  value={String(docData.customer_contact_id || '')}
                  onValueChange={(v) => setDocData({ ...docData, customer_contact_id: Number(v) })}
                  disabled={!docData.customer_id || contactsList.length === 0}
                >
                  <SelectTrigger className="h-11 bg-admin-bg border-admin-border rounded-xl text-sm focus:ring-zeronix-blue/10 transition-all shadow-sm">
                  ...
                  <SelectContent className="bg-admin-surface border-admin-border rounded-2xl shadow-2xl">
                    {contactsList.map((c) => (
                      <SelectItem key={c.id} value={String(c.id)} className="hover:bg-admin-surface-hover rounded-lg m-1">
                        <span className="font-bold text-admin-text-primary">{c.full_name}</span>
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-wider text-admin-text-muted ml-1 flex items-center gap-1.5">
                  <Edit3 size={12} className="text-zeronix-blue" /> Status
                </Label>
                <Select value={String(docData.status || '')} onValueChange={(v) => setDocData({ ...docData, status: v })}>
                  ...
                  <SelectContent className="bg-admin-surface border-admin-border rounded-2xl shadow-2xl">
                    {['draft', 'confirmed', 'processing', 'completed', 'cancelled'].map(s => <SelectItem key={s} value={s} className="rounded-lg m-1">{s.toUpperCase()}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-wider text-admin-text-muted ml-1 flex items-center gap-1.5">
                  <Calendar size={12} className="text-zeronix-blue" /> Order Date
                </Label>
                <Input
                  type="date"
          ...
          <div className="bg-admin-surface border border-admin-border rounded-lg overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-admin-border flex items-center justify-between bg-admin-bg/10">
              <span className="text-xs font-semibold text-admin-text-primary uppercase tracking-wider">Line Items</span>
              <Button
                size="sm"
                variant="outline"
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="h-40 text-center">
                        <div className="flex flex-col items-center gap-2 text-admin-text-muted opacity-40">
                          <Plus size={32} />
                          <p className="text-[10px] font-bold uppercase tracking-wider">No line items yet</p>
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : items.map((item, i) => (
                    <TableRow key={i} className="border-admin-border group hover:bg-admin-surface-hover/50 transition-colors">
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Button variant="ghost" size="icon" onClick={() => { setEditingIndex(i); setIsModalOpen(true); }} className="h-8 w-8 text-admin-text-muted hover:text-zeronix-blue hover:bg-zeronix-blue/10 rounded-lg">
                            <Pencil size={14} />
                          </Button>
