  ...
  if (loading && !isNew) {
    return (
      <div className="h-96 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-[10px] font-semibold uppercase tracking-wider text-brand-subtle">Loading Data...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300 pb-20 md:pb-0">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        <div className="xl:col-span-8 space-y-6">
          <div className="bg-brand-white border border-brand-border rounded-lg p-6 shadow-sm space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Select value={String(docData.customer_id || '')} onValueChange={(v) => setDocData({ ...docData, customer_id: Number(v) })} disabled={isLocked}>
                  ...
                  <SelectContent className="bg-brand-white border-brand-border rounded-2xl shadow-2xl">
                    {customersList.map((c) => (
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-wider text-brand-subtle ml-1 flex items-center gap-1.5">
                  <Calendar size={12} className="text-brand-accent" /> Delivery Date
                </Label>
                <Input
                  type="date"
                  disabled={isLocked}
                  ...
                  className="h-11 bg-brand-bg border-brand-border rounded-xl text-sm shadow-sm"
                />
              </div>
              ...
              {!isNew && (
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase tracking-wider text-brand-subtle ml-1 flex items-center gap-1.5">
                    <Truck size={12} className="text-brand-accent" /> Linked Sales Order
                  </Label>
                  <p className="h-11 flex items-center px-3 bg-brand-bg border border-brand-border rounded-xl text-sm text-brand-secondary">
              ...
              {!isNew && (
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase tracking-wider text-brand-subtle ml-1 flex items-center gap-1.5">
                    <FileCheck2 size={12} className="text-brand-accent" /> Linked Invoice
                  </Label>
                  <p className="h-11 flex items-center px-3 bg-brand-bg border border-brand-border rounded-xl text-sm text-brand-secondary">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="bg-brand-bg border-brand-border rounded-xl text-sm resize-none min-h-[70px]"
              />
            </div>
              ...
              <div className={`rounded-xl border p-4 flex gap-3 ${docData.customer_confirmation === 'accepted' ? 'bg-emerald-500/5 border-emerald-500/30' : 'bg-danger/5 border-danger/30'}`}>
                ...
                <div className="space-y-1">
                  ...
                  {docData.customer_notes && (
                    <p className="text-xs text-brand-secondary italic">"{docData.customer_notes}"</p>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="bg-brand-white border border-brand-border rounded-lg overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-brand-border flex items-center justify-between bg-brand-bg/10">
              <span className="text-xs font-semibold text-brand-primary uppercase tracking-wider">Items</span>
              {!isLocked && (
                <Button size="sm" variant="outline" onClick={() => setItems([...items, { product_id: undefined, product_name: '', quantity: 1 }])} className="h-8 rounded-md text-xs border-brand-border bg-brand-white hover:bg-brand-bg px-3">
              ...
              <Table>
                <TableHeader className="bg-brand-bg/30">
                  <TableRow className="border-brand-border hover:bg-transparent">
                    <TableHead className="w-12 text-center text-[11px] font-semibold uppercase tracking-wider text-brand-subtle">#</TableHead>
                    <TableHead className="min-w-[260px] text-[11px] font-semibold uppercase tracking-wider text-brand-subtle">Product</TableHead>
                    <TableHead className="text-center text-[11px] font-semibold uppercase tracking-wider text-brand-subtle w-28">Quantity</TableHead>
                ...
                <TableBody>
                  {items.map((item, i) => (
                    <TableRow key={i} className="border-brand-border">
                      <TableCell className="text-center text-xs text-brand-subtle font-medium">{i + 1}</TableCell>
                      <TableCell className="py-2">
                        {isLocked ? (
                          <p className="text-sm text-brand-primary">{item.product_name}</p>
                        ) : (
                          <Select
                          ...
                          >
                            <SelectTrigger className="h-9 bg-brand-bg border-brand-border rounded-lg text-sm">
                      ...
                      <TableCell className="text-center">
                        {isLocked ? (
                          <span className="text-sm text-brand-secondary">{item.quantity}</span>
                        ) : (
                          <Input
                            type="number"
                            min={0.01}
                            ...
                            className="h-9 bg-brand-bg border-brand-border rounded-lg text-sm text-center"
                          />
                        )}
                      </TableCell>
                      <TableCell className="text-right">
                        {!isLocked && (
                          <Button variant="ghost" size="icon" onClick={() => setItems(items.filter((_, idx) => idx !== i))} className="h-8 w-8 text-brand-subtle hover:text-danger hover:bg-danger/10 rounded-lg">
                            <Trash2 size={14} />
                          </Button>
                        )}
                      </TableCell>
